...
def __get_remote_index(warc_files_start_date=None, warc_files_end_date=None):
    ...
    try:
        ...
        __logger.info(
            "Failed to read %s bucket, using monthly WARC file listings", __cc_bucket
        )
        s3_client = None
    ...
    if s3_client:
        ...
        if warc_files_start_date or warc_files_end_date:
            ...
            for date in warc_dates:
                ...
                __logger.debug(
                    "Listing objects on S3 bucket %s and prefix %s", __cc_bucket, prefix
                )
        ...
        for date in warc_dates:
            year = date.strftime("%Y")
            month = date.strftime("%m")
            url = "%scrawl-data/CC-NEWS/%s/%s/warc.paths.gz" % (
                __cc_base_url,
                year,
                month,
            )
