...
class ComparerText:
    """This class compares the text of the list of
    ...
    def extract(self, item, article_candidate_list):
        ...
        :param article_candidate_list: A list, the
            list of ArticleCandidate-Objects which have been extracted
        :return: A string, the most likely text
        """
        list_text = []

        # The texts of the article candidates and the
        #   respective extractors are saved in a tuple in list_text.
        ...
        list_text = list(
            filter(lambda x: len(x[0].split()) >= MIN_NUMBER_WORDS, list_text)
        )
        ...
        for (
            a,
            b,
        ) in itertools.combinations(list_text, 2):
            ...
            # Create the score. It divides the number of words
            # which are not in both texts by the number of words which
            # are in both texts and subtracts the result from 1.
            # The closer to 1 the more similiar they are.
        ...
        # A list that holds the extracted texts and
        # their extractors which were most similar
        top_candidates = []
        for tup in list_text:
            if tup[1] == best_score[1] or tup[1] == best_score[2]:
                top_candidates.append(tup)
